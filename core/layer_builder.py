...
class LayerBuilder:
    @staticmethod
    def build_layers(asset: dict, geo_fields: List[GeoField], submissions: list) -> List[QgsVectorLayer]:
        uid = asset.get("uid", "")
        ...
        for geo_field in geo_fields:
            ...
            layer.updateFields()
            LayerBuilder._add_features(layer, geo_field, submissions, layer.fields())
    ...
    def _build_fields(submissions: list, geo_field_names: set) -> List[QgsField]:
        ...
        fields = []
        seen = set()
        # Always include kobo_id and kobo_uuid first
        ...
        for key in first.keys():
            ...
            if key.startswith("_"):
                ...
                continue
            seen.add(key)
            name = key[:50]  # truncate long field names
            fields.append(QgsField(name, _STR))
        return fields

    @staticmethod
    def _add_features(layer: QgsVectorLayer, geo_field: GeoField, submissions: list, qgs_fields: QgsFields) -> None:
        features = []
